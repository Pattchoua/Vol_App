...
const updateOrganizationProfile = async (req, res) => {
  try {
    ...
    console.log(
      "🚀 ~ file: organizations.js:55 ~ updateOrganization ~ updatedOrganization:",
      updatedOrganization
    );
    ...
    console.log(
      "🚀 ~ file: organizations.js:81 ~ updateOrganization ~ error:",
      error
    );
...
const getProjectsCreatedByOrganization = async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ error: "Error fetching projects created by organization" });
  }
};
...
const getVolunteersByOrganization = async (req, res) => {
  try {
    ...
    const orgaProjects = await Project.find({
      organizationId: req.organization._id,
    });
    ...
    res
      .status(500)
      .json({ error: "Error fetching volunteers by organization" });
  }
};
...
const getProjectByIdCreatedByOrganization = async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ error: "Error fetching a project created by organization" });
  }
};
...
const updateProjectDecision = async (req, res) => {
  ...
  try {
    const updatedProject = await Project.changeProjectStatusBasedOnDecision(
      projectId,
      decision
    );
    ...
    res
      .status(200)
      .json({ message: "Project decision updated", project: updatedProject });
  } catch (error) {
    console.error(error);
...
const getLoggedinOrganization = async (req, res) => {
  try {
    const organization = await Organization.findOne({
      ...
      "_id email organizationName description contactInfo website projects decision"
    );
    console.log(
      "🚀 ~ file: auth-organization.js:87 ~ getLoggedinOrganization ~ organization:",
      organization
    );
    res.json({ organization });
  } catch (error) {
    console.log(
      "🚀 ~ file: auth-organization.js:90 ~ getLoggedinOrganization ~ error:",
      error
    );
